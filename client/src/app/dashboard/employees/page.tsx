...
import { useGetAllEmployees, useDeleteEmployeeById } from '@/features/useEmployees';
...
const Page: React.FC = () => {
  ...
  const { data, isLoading, isError, refetch } = useGetAllEmployees(page, debouncedSearch);
  const { mutateAsync: deleteEmployee, isPending: isDeleting } = useDeleteEmployeeById();

  const { employees, totalPages, hasNextPage, hasPreviousPage, currentPage } =
    useMemo(() => {
      const employees: EmployeeEntity[] = data?.data.items || [];
  ...
  const handleDelete = async (id: ID, name: string) => {
    showWarning({
      message: `هل أنت متأكد من حذف الموظف "${name}" نهائياً؟`,
      btn1: 'إغلاق',
      btn2: 'حذف',
      handleBtn2: () => deleteEmployee(id),
    });
  };

  const handleEdit = (id: ID) => router.push(`employees/edit/${id}`);
  const handleAdd = () => router.push('employees/add');
    ...
    <div className="space-y-6">
      ...
      <PageLayout className="space-y-6">
        ...
        <Contents
          ...
          emptyDesc={
            debouncedSearch
              ? 'لم يتم العثور على موظفين مطابقين لبحثك.'
              : "لا يوجد موظفون بعد. يمكنك إضافة موظف جديد من خلال الضغط على زر 'إضافة موظف'."
          }
          isError={isError}
          errorTitle="خطأ في تحميل البيانات"
          errorDesc="حدثت مشكلة أثناء تحميل قائمة الموظفين. يرجى المحاولة لاحقاً."
              ...
              <table className="w-full text-right font-sans">
                <thead>
                  <tr className="border-b border-border bg-background/60 font-heading text-xs font-semibold text-muted">
                    ...
                    <th className="px-5 py-4">الدور الوظيفي</th>
